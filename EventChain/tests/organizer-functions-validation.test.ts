...
describe("Organizer Functions - Validation Tests", () => {
  it("should validate that organizer-specific functions work correctly", () => {
    ...
    simnet.callPublicFn("eventchain", "add-organizer", [Cl.principal(org1)], deployer);
    simnet.callPublicFn("eventchain", "add-organizer", [Cl.principal(org2)], deployer);
    simnet.callPublicFn("eventchain", "add-organizer", [Cl.principal(org3)], deployer);

    // Get baseline counts for all organizers
    const baselineCount1 = simnet.callReadOnlyFn("eventchain", "get-organizer-events-count", [Cl.principal(org1)], deployer);
    const baselineCount2 = simnet.callReadOnlyFn("eventchain", "get-organizer-events-count", [Cl.principal(org2)], deployer);
    const baselineCount3 = simnet.callReadOnlyFn("eventchain", "get-organizer-events-count", [Cl.principal(org3)], deployer);
    ...
    const org1Event1 = simnet.callPublicFn("eventchain", "create-event", [
    ...
    const org1Event2 = simnet.callPublicFn("eventchain", "create-event", [
    ...
    const org2Event1 = simnet.callPublicFn("eventchain", "create-event", [
    ...
    // Get final counts
    const finalCount1 = simnet.callReadOnlyFn("eventchain", "get-organizer-events-count", [Cl.principal(org1)], deployer);
    const finalCount2 = simnet.callReadOnlyFn("eventchain", "get-organizer-events-count", [Cl.principal(org2)], deployer);
    const finalCount3 = simnet.callReadOnlyFn("eventchain", "get-organizer-events-count", [Cl.principal(org3)], deployer);
    ...
    const org1Increase = Number(finalCount1.result.value) - Number(baselineCount1.result.value);
    const org2Increase = Number(finalCount2.result.value) - Number(baselineCount2.result.value);
    const org3Increase = Number(finalCount3.result.value) - Number(baselineCount3.result.value);
    ...
    const org1Events = simnet.callReadOnlyFn("eventchain", "get-organizer-events", [Cl.principal(org1)], deployer);
    const org2Events = simnet.callReadOnlyFn("eventchain", "get-organizer-events", [Cl.principal(org2)], deployer);
    const org3Events = simnet.callReadOnlyFn("eventchain", "get-organizer-events", [Cl.principal(org3)], deployer);
    ...
    if (org1Events.result.type === 11) {
      ...
      expect(org1Events.result.list.length - Number(baselineCount1.result.value)).toBe(2);
    }

    if (org2Events.result.type === 11) {
      // Org2 should have gained 1 more event
      expect(org2Events.result.list.length - Number(baselineCount2.result.value)).toBe(1);
    }

    if (org3Events.result.type === 11) {
      // Org3 should have the same number of events as baseline
      expect(org3Events.result.list.length).toBe(Number(baselineCount3.result.value));
    }
  ...
  it("should validate organizer functions work with non-existent organizer", () => {
    ...
    // Non-existent organizer should always return 0 count and empty list
    expect(count.result).toStrictEqual(Cl.uint(0));
    expect(events.result).toStrictEqual(Cl.list([]));
